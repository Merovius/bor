...
class AccountsListTest : public CppUnit::TestFixture {
    CPPUNIT_TEST_SUITE(AccountsListTest);
...
CPPUNIT_TEST_SUITE_REGISTRATION(AccountsListTest);

void AccountsListTest::setUp() {
    bank1 = new Bank("TestBank", "list");
    beate = bank1->createAccount("Beate");
    harald = bank1->createAccount("Harald");
    bank2 = new Bank("TestBank2", "list");
    charlie = bank2->createAccount("Charlie");
    srand(time(NULL));
}

void AccountsListTest::TakeEmpty() {
...
void AccountsListTest::TakeNonExistent() {
    int acc;
...
void AccountsListTest::PutMoney() {
...
void AccountsListTest::TakeMoney() {
...
void AccountsListTest::TransferMoney() {
...
void AccountsListTest::TransferBetweenBanks() {
    int balanceBeate = bank1->balance(beate);
...
void AccountsListTest::tearDown() {
    delete bank1;
    delete bank2;
}
